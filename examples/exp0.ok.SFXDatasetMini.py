...
import random
import numpy as np
from peaknet.datasets.images import ConfigDataset, SFXDatasetMini
...
class VizCheetahGeom:
    def __init__(self, geom_dict, img, mask, found_dict, indexed_dict, peak_list, title, figsize, **kwargs):
    ...
    def plot_found(self):
        found_dict = self.found_dict
        ax_img = self.ax_list[0]
        offset = 4
        for panel, peak_list in found_dict.items():
...
dataset_train = SFXDatasetMini(config_dataset)
dataset_train.cache_img()

# Let's plot the first image...
idx = 0
fl_stream, fl_cxi, event_crysfel = dataset_train.metadata_list[idx]
img, mask = dataset_train[idx]

stream_dict = dataset_train.stream_cache_dict[fl_stream]
geom_dict = stream_dict['geom']
found_dict, indexed_dict = dataset_train.get_raw_peak(idx)
