...
class SanityChecker:
    ...
    def plot_mask_predicted_overlay(self):
        img            = self.img_true
        mask_predicted = self.mask_predicted
        ...
        rec_obj = mpatches.Rectangle((0, 0), 
                                     2 * offset, 2 * offset, 
                                     linewidth = self.linewidth, 
                                     edgecolor = 'green', 
                                     facecolor='none')
    ...
    def plot_mask_predicted(self):
        ...
        adu_threshold = self.adu_threshold
        mask[  mask_predicted < adu_threshold ] = 0
        ## mask[~(mask_predicted < adu_threshold)] = 1e4

        ax_img = self.ax_list[2]
        im = ax_img.imshow(mask, vmin = 0, vmax = 1, cmap = 'gray')
        ## im.set_cmap('gray')
        ax_img.invert_yaxis()
