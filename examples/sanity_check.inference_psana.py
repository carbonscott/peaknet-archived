...
class SanityChecker:
    ...
    def create_panels(self):
        ...
        gspec = fig.add_gridspec( nrows, ncols,
                                  width_ratios  = [1/2, 1/2, 1/20],
                                  height_ratios = [1/3, 1/3, 1/3], 
                                )
        ax_list = [ fig.add_subplot(gspec[0, 0:2], aspect = 1) ]
        ax_list.append( fig.add_subplot(gspec[1, 0:2], aspect = 1) )
        ax_list.append( fig.add_subplot(gspec[2, 0:2], aspect = 1) )
        ax_list.append( fig.add_subplot(gspec[:, 2], aspect = 1) )
...
fl_pdf = f'pf.inference'
disp_manager.show()
## disp_manager.show(linewidth = 0.1, filename = fl_pdf)
